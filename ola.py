...
}  # <-- EDITA COORDENADAS AQUÍ

# ==================== BONO AUTOMÁTICO POR HORA DE CITA ====================
def calcular_bono_transporte(hora_cita) -> int:
    """
    Retorna el bono de transporte según la hora de cita:
    - 06:00 exactas            → $10.000
    - 19:00 a 21:00 (9pm)     → $10.000
    - 21:30 a 23:00            → $20.000
    - Resto                    → $0
    """
...
class DB:
    ...
    def init(self):
        try:
            ...
            migraciones = [
                ...
                "ALTER TABLE jp_viajes ADD COLUMN IF NOT EXISTS combustible NUMERIC(12,0) DEFAULT 0",
        ...
        try:
            ...
            cur.execute("""
                ...
                (fecha, placa, conductor, cliente, origen, destino,
                 ...
                 flete, comision, bono_transporte, combustible)
    ...
    def actualizar_viaje(self, viaje_id: int, datos: dict) -> bool:
        try:
            ...
            cur.execute("""
                ...
                flete=%s, comision=%s, bono_transporte=%s, combustible=%s
                WHERE id=%s
            """, (
                datos["fecha"], datos["placa"], datos["conductor"], datos["cliente"],
                datos["origen"], datos["destino"],
    ...
    def obtener_viajes(self, fecha_ini=None, fecha_fin=None, placa=None,
        ...
        q = """SELECT id, fecha, placa, conductor, cliente, origen, destino,
                      ...
                      COALESCE(combustible,0) as combustible
...
def generar_excel(df: pd.DataFrame, titulo: str = "JP Transportamos") -> bytes:
    ...
    cols_dinero  = ["FLETE","COMISIÓN","BONO TRANSP.","COMBUSTIBLE","UTILIDAD NETA"]
    ...
    tot_flete = tot_comision = tot_bono = tot_combustible = 0
    ...
    for row_idx, (_, fila) in enumerate(df.iterrows(), start=3):
        ...
        tot_bono       += bono_v
        tot_combustible += combustible_v

        fill_d = fill_dinero if fill_f is None else fill_f
        for ci, (val, es_neg) in enumerate([
            (flete_v, False),(comision_v, True),(bono_v, True),(combustible_v, True),(utilidad_v, utilidad_v < 0)
        ], start=col_d_start):
            c = ws.cell(row=row_idx, column=ci, value=val)
            c.font = Font(name="Calibri", size=9, color="C0392B" if es_neg and val < 0 else ("1E8449" if ci == col_d_start + 4 else "000000"))
    ...
    for ci, (val, lbl) in enumerate([
        (tot_flete,"TOT.FLETE"),(tot_comision,"TOT.COMIS."),(tot_bono,"TOT.BONO"),
        (tot_combustible,"TOT.COMB."),(tot_utilidad,"TOT.UTILIDAD")
    ], start=col_d_start):
        c = ws.cell(total_row, ci, val)
        c.font = Font(name="Calibri", bold=True, size=10,
                      color="1E8449" if ci == col_d_start + 4 and val >= 0 else ("C0392B" if val < 0 else "000000"))
    ...
    hdrs_c = ["CONDUCTOR","VIAJES","COMPLET.","ANULADOS","INCUMPL.","% CUMPL.",
              "TOT. FLETE","TOT. COMISIÓN","TOT. BONO","TOT. COMBUSTIBLE","UTILIDAD NETA"]
    ...
    hdrs_v = ["PLACA","VIAJES","COMPLET.","TOT. FLETE","TOT. COMBUSTIBLE","UTILIDAD","CONDUCTORES"]
    ...
    if "placa" in df.columns:
        ...
        for i, r in enumerate(df_veh.itertuples(), start=3):
            ...
            vals = [r.placa, r.viajes, r.comp, int(r.t_flete), int(r.t_comb), int(r.utilidad), r.conductores]
            for ci2, v in enumerate(vals, start=1):
                c = ws_v.cell(i, ci2, v)
                c.font = ft_normal; c.border = borde
                c.alignment = izq if ci2 in (1,7) else centro
                if ci2 in (4,5,6): c.number_format = '#,##0'
                if fill_vi: c.fill = fill_vi

    for col_l, w in zip(["A","B","C","D","E","F","G"],[12,8,10,14,16,14,30]):
...
def main():
    ...
    <div class="main-header">
        ...
        <p class="subtitle">Control de Viajes · Comisiones · Trazabilidad</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with tab1:
        ...
        # --- Bono automático según hora de cita ---
        ...
        if bono_auto > 0:
            ...
            st.markdown(
                f"<div style='margin:6px 0;'>🌙 Bono de transporte: <span class='bono-zero'>$0 — hora normal</span></div>",
                unsafe_allow_html=True
            )
    ...
    with tab2:
        ...
        if not df.empty:
            ...
            k6.metric("📈 Utilidad",     fmt_moneda(
                df["flete"].sum() - df["comision"].sum() - df["bono_transporte"].sum() - df["combustible"].sum()
            ))
            ...
            if sel:
                ...
                if not editando:
                    ...
                    comb_r = int(row.get("combustible",0) or 0)
                    widget_liquidacion(flete_r, comision_r, bono_r, comb_r)
                    ...
                    if guardar:
                        datos_edit = {
                            ...
                            "bono_transporte": bono_edit, "combustible": e_combustible,
                        }
    ...
    with tab3:
        ...
        try:
            ...
            k5.metric("⛽ Combustible", fmt_moneda(tot_comb_d))
            ...
            g5, g6 = st.columns(2)
            with g5:
                st.markdown("#### ⛽ Combustible vs Flete por Vehículo")
                df_vf = df_s.groupby("placa").agg(flete=("flete","sum"), comb=("combustible","sum")).reset_index()
                fig5 = px.bar(df_vf, x="placa", y=["flete","comb"],
                              barmode="group",
                              color_discrete_map={"flete":"#2ecc71","comb":"#e74c3c"})
    ...
    with tab4:
        ...
        with traza_tab1:
            ...
            if not df_cond_t.empty:
                ...
                r2c2.metric("⛽ Total Combustible", fmt_moneda(comb_c))
                r2c3.metric("📈 Utilidad Neta", fmt_moneda(utilidad_c))
                prom_flete = int(flete_c / total_c) if total_c > 0 else 0
                r2c4.metric("📊 Flete Promedio", fmt_moneda(prom_flete))
                ...
                cols_det = ["fecha","placa","cliente","origen","destino","estado",
                            "flete","comision","bono_transporte","combustible","numero_factura"]
                ...
                # Descarga Excel solo del conductor
        ...
        with traza_tab2:
            ...
            if not df_veh_t.empty:
                ...
                comb_v     = int(df_veh_t["combustible"].sum())
                ...
                vk3.metric("⛽ Combustible", fmt_moneda(comb_v))
                vk4.metric("👥 Conductores que lo manejaron", len(conductores_v))
                ...
                cols_v = ["fecha","conductor","cliente","origen","destino","estado",
                          "flete","combustible","numero_factura","observacion"]
